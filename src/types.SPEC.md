...
- The data shapes: an installed skill package (npm name, version, location, skill name), the contents of `source.json` (package, version, content hash), the config settings, the mirroring analysis (physical skills directories, mirror style symlink-or-copy, primary directory), and the sync result (project root, target directories, analysis, per-skill outcomes, warnings, exit code).
